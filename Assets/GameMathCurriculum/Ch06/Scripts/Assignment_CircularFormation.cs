...
public class Assignment_CircularFormation : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < units.Length; i++)
        {
            ...
            Vector3 offset = Quaternion.AngleAxis(angle, leader.up) * leader.forward * formationRadius;
            units[i].position = leader.position + offset;

            Vector3 dir = units[i].position - leader.position;
            units[i].rotation = Quaternion.LookRotation(dir, leader.up);
