...
public class Assignment_SplineConveyor : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < boxes.Length; i++)
        {
            ...
            float boxT = globalT + spacing * i;
            boxT = Mathf.Repeat(boxT, 1f);
            Vector3 position =  EvaluateSpline(waypoints, boxT);
            boxes[i].position = position;

            float lookAheadT = Mathf.Repeat(boxT + 0.01f, 1f);
            Vector3 nextPosition = EvaluateSpline(waypoints, lookAheadT);

            Vector3 dir = (nextPosition - position).normalized;

            if (dir != Vector3.zero)
            {
                boxes[i].rotation = Quaternion.LookRotation(dir, Vector3.up);
            }
